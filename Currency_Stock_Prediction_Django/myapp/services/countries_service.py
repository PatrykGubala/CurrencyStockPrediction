...
class CountriesService:
    def __init__(self):
        self.countries_repository = CountriesRepository()
    ...
    def load_only_countries(self, countries_data: List[dict]):
        for country in countries_data:
            ...
            if country_code and country_name:
                existing_country = self.countries_repository.get_country_by_code(country_code)
                if not existing_country:
                    self.countries_repository.add_country(country_code, country_name)
    ...
    def load_countries_with_details(self, countries_data: List[dict]):
        for country in countries_data:
            ...
            existing_country = self.countries_repository.get_country_by_code(country_code)
            if not existing_country:
                existing_country = self.countries_repository.add_country(country_code, country_name)

            region_name = country.get('region')
            if region_name:
                region = self.regions_repository.get_region_by_name(region_name)
                if not region:
                    region = self.regions_repository.add_region(region_name)
                self.countries_repository.associate_region_to_country(existing_country, region)
            ...
            for cdata in currencies:
                ...
                if code and cname:
                    ...
                    if not currency:
                        currency = self.currencies_repository.add_currency(code, cname, symbol)
                    self.countries_repository.associate_currency_to_country(existing_country, currency)

    def get_all_countries_dto(self) -> List[dict]:
        countries = self.countries_repository.get_all_countries()
        result = []
    ...
    def get_country_by_code_dto(self, country_code: str) -> Optional[dict]:
        country = self.countries_repository.get_country_by_code(country_code)
        if not country:
            return None
        return {
            "id": country.id,
