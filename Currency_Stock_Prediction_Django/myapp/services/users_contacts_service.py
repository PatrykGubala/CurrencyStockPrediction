...
class UsersContactsService:
    ...
    def create_contact(self, user, title: str, public_account_id: str, account_name: str, currency_code: str):
        if not self.contacts_repo.account_exists(public_account_id):
            raise ValueError("Public account ID does not exist.")
        return self.contacts_repo.add_contact(user, title, public_account_id, account_name, currency_code)