...
from myapp.repositories.currency_pairs_data_repository import CurrencyPairsDataRepository
import logging

logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)

class CurrencyPairsDataService:
    def __init__(self):
        self.repository = CurrencyPairsDataRepository()

    def load_hourly_data(self, pair_ids=None):
        logger.info("Starting hourly data load process")
        pairs = CurrencyPair.objects.all()
        if pair_ids:
            pairs = pairs.filter(id__in=pair_ids)
        for pair in pairs:
            ...
            if hourly_data is not None:
                ...
                if processed_hourly:
                    self.repository.bulk_upsert_data(pair, processed_hourly)
                    pair.data_availability = True
                    pair.save()
                    logger.info(f"Successfully loaded hourly data for pair: {pair.name}")
                else:
                    logger.warning(f"No valid hourly data to process for pair: {pair.name}")
            else:
                logger.warning(f"No hourly data fetched for pair: {pair.name}")
        logger.info("Hourly data load process completed")

    def load_daily_data(self, pair_ids=None):
        ...
        for pair in pairs:
            ...
            if daily_data is not None:
                ...
                if processed_daily:
                    self.repository.bulk_upsert_data(pair, processed_daily)
                    pair.data_availability = True
                    pair.save()
                    logger.info(f"Successfully loaded daily data for pair: {pair.name}")
                else:
                    logger.warning(f"No valid daily data to process for pair: {pair.name}")
            else:
                logger.warning(f"No daily data fetched for pair: {pair.name}")
        logger.info("Daily data load process completed")

    def fetch_data(self, pair: CurrencyPair, frequency: str, start_date: str, end_date: str) -> Optional[pd.DataFrame]:
        try:
            ticker = yf.Ticker(pair.name)
            hist = ticker.history(start=start_date, end=end_date, interval=frequency)
            if hist.empty:
                logger.warning(f"{pair.name}: No data returned from yfinance for interval: {frequency} between {start_date} and {end_date}")
                return None
            hist = hist.reset_index()
            if 'Datetime' in hist.columns:
                hist['Datetime'] = pd.to_datetime(hist['Datetime'], utc=True)
                hist = hist[(hist['Datetime'].dt.hour >= 8) & (hist['Datetime'].dt.hour < 24)]
                hist['Datetime'] = hist['Datetime'].dt.floor('h')  # Changed from 'H' to 'h'
            elif 'Date' in hist.columns:
                hist['Date'] = pd.to_datetime(hist['Date'], utc=True)
                hist['Date'] = hist['Date'].dt.floor('D')
            logger.info(f"Data fetched for pair: {pair.name} with {len(hist)} rows and interval: {frequency}")
            return hist
        except Exception as e:
            logger.error(f"Error fetching data for pair {pair.name} with interval {frequency}: {e}")
            return None
    ...
    def get_all_data(self) -> List[dict]:
        result = []
        for data in CurrencyPairData.objects.select_related('currency_pair').order_by('-id'):
            result.append({
                'pair_id': data.currency_pair.id,
    ...
    def get_latest_data_for_pair(self, pair_id: int) -> Optional[dict]:
        pair = CurrencyPair.objects.filter(id=pair_id).first()
        if not pair:
            return None
        latest = self.repository.get_latest_record(pair)
        if not latest:
            return None
        return {
            'pair_id': pair.id,
    ...
    def get_percentage_change(self, pair_id: int) -> Optional[dict]:
        pair = CurrencyPair.objects.filter(id=pair_id).first()
        if not pair:
            return None
        latest = self.repository.get_latest_record(pair)
        if not latest:
            return None
        previous = self.repository.get_previous_record(pair, latest.timestamp)
        if not previous:
            return None
